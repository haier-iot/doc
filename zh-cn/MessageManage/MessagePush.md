...
> 实现将端的信息绑定到消息中心的功能，在调用本接口后可授权端使用消息中心推送通道向其它端推送消息。</br>
...
##### 按模板向端推送消息
> 判断消息发送端与接收端是否在消息中心注册成功，成功则通过相应的通道推送消息</br>
> 消息发送端与接收端必须先在消息中心注册后方可使用消息中心的发送通道推送消息。</br>
> 调用方需要提前调用注册接口在消息中心注册终端</br>
